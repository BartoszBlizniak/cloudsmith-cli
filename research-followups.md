...
PoC branch: `poc/pex-scie` on `BartoszBlizniak/cloudsmith-cli`.
Workflow: `.github/workflows/poc-pex-scie.yml`.
...
**Why it matters:** `cloudsmith-cli-action` needs to pick the right artefact per host. Falling back to glibc when both detection methods fail risks downloading a binary that won't run on Alpine; failing hard risks blocking a workflow that doesn't need libc detection at all.

**Plan:**
1. Implement the resolution algorithm in `cloudsmith-cli-action/src/download-cli.js` (from `research.md` §6).
2. Add unit tests that mock `execSync('ldd --version')` and `fs.existsSync('/etc/os-release')` returning various error shapes.
3. Run integration tests across: `ubuntu:latest`, `alpine:latest`, `gcr.io/distroless/static-debian12`, `busybox:latest`, `chainguard/static:latest`, `cgr.dev/chainguard/wolfi-base`.
4. Decide: glibc default on detection failure, or hard error.

**Expected outcome:** A robust resolver with tests + a documented fallback policy.

**Agent prompt:**
> Implement and test libc detection in `cloudsmith-cli-action` for the Cloudsmith CLI binary distribution. Pseudocode is in `research.md` §6 of the spike research (in the `cloudsmith-cli` repo). The detection runs in Node inside the GitHub Action: `process.platform`, `process.arch`, plus on Linux `execSync('ldd --version 2>&1')` checked for "musl" stderr, with fallback to `fs.readFileSync('/etc/os-release')` checked for "Alpine". Step 1: implement in `cloudsmith-cli-action/src/download-cli.js` (currently downloads only `.pyz` and wraps in `cloudsmith.bat` on Windows — see `https://github.com/cloudsmith-io/cloudsmith-cli-action/blob/master/src/download-cli.js`). Step 2: add unit tests that mock both detection methods with various error shapes. Step 3: integration-test across `ubuntu:latest`, `alpine:latest`, `gcr.io/distroless/static-debian12`, `busybox:latest`, `chainguard/static:latest`, `cgr.dev/chainguard/wolfi-base`. Step 4: decide on a fallback policy when both detections fail — recommend "default to glibc" but be ready to switch if testing shows it breaks too many minimal-image workflows. Deliverable: a PR to `cloudsmith-cli-action` with the resolver + tests + a `README.md` section documenting the resolution flow. Maintain backwards-compat: on a 404 from the binary URL, fall back to the current `.pyz` + `actions/setup-python` flow.
